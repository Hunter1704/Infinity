...
export const getSolvedStats = (checkedProblems, problemTotals) => {
  if (!checkedProblems)
    checkedProblems = [];

  const solvedStats = {
    basic: 0, totalBasic: problemTotals?.Basic || 0,
    easy: 0, totalEasy: problemTotals?.Easy || 10,
    medium: 0, totalMedium: problemTotals?.Medium || 8,
    hard: 0, totalHard: problemTotals?.Hard || 7,
    total: 0, totalOverall: problemTotals?.Total || 25,
  };
...
export const mapUserInfo = (userInfo) => {
  const user = {
    ...
    solvedStats: getSolvedStats(userInfo.checkedProblems, userInfo.problemTotals),
